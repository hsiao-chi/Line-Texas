...
func callbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, result := range received.Results {
		...
		if content != nil && content.IsMessage && content.ContentType == linebot.ContentTypeText {
			...
			_, err = bot.SendSticker([]string{content.From}, 3, 1, 100)
			_, err = bot.SendSticker([]string{content.From}, 4, 1, 100)
			_, err = bot.SendSticker([]string{content.From}, 5, 1, 100)
			_, err = bot.SendSticker([]string{content.From}, 6, 1, 100)
